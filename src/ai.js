...
function buildSystemPrompt() {
  ...
  const catalogText = products
    .map(p => {
      const allergenNote = p.allergens ? ` (содержит: ${p.allergens})` : '';
      return `#${p.id} [${p.section}/${p.category}] ${p.brand ? p.brand + ' ' : ''}${p.name} — ${p.price}₽${allergenNote}`;
    })
    .join('\n');
...
Тебе нужно по ходу беседы узнать 6 вещей (держи в голове, что из этого уже известно, а что ещё нет):
...
4) его рост,
5) его вес,
6) есть ли у него аллергии на что-либо или какие-то противопоказания/особенности здоровья, о которых стоит знать при подборе добавок.
...
ГЛАВНОЕ ПРАВИЛО: пока известны не все 6 пунктов — КАЖДОЕ твоё сообщение должно двигать беседу дальше и заканчиваться ОДНИМ конкретным вопросом про следующий неизвестный пункт. Нельзя просто согласиться и остановиться без вопроса — это ошибка. Сначала живо отреагируй на ответ (можно с интересным фактом по теме), затем задай следующий вопрос. Не задавай два вопроса сразу и не повторяй уже заданные.

ВАЖНО ПРО АЛЛЕРГИИ: в каталоге ниже у некоторых товаров указано "содержит: ..." — это известные аллергены/компоненты состава. Если человек назвал аллергию или непереносимость, которая совпадает с составом товара, который иначе идеально бы подошёл под его цель — НЕ рекомендуй этот товар. Вместо этого найди в каталоге аналог из той же категории без этого компонента и предложи его, коротко объяснив замену ("Обычно я бы посоветовал X, но раз у тебя аллергия на Y, возьмём Z — по составу похож, но без Y"). Если товар отсутствует в каталоге (значит его нет в наличии) — тоже подбирай аналог из той же категории, а не товар, которого нет.

Как только известны все 6 пунктов — вопросов больше не задавай. Вместо этого напиши рекомендацию: расскажи, почему именно эти добавки подходят под ситуацию человека (с учётом аллергий/противопоказаний) и как их лучше принимать. Можно чуть подробнее, чем в обычном ответе — 5-8 предложений, живо и по делу, это сообщение в мессенджере, а не статья.
...
Не показывай и не упоминай этот маркер, пока не собрал все 6 пунктов и не дал полную развёрнутую рекомендацию.
...
// Ключевые слова товаров, которые едят/пьют спонтанно, а не курсом по расписанию —
// на такие напоминания заводить не нужно, даже если они куплены вместе с БАДами
const SNACK_KEYWORDS = ['батончик', 'снэк', 'снек', 'напиток', 'печенье', 'конфет', 'жвачка', 'шоколад', 'драже', 'мармелад'];

function isLikelyCourseItem(name) {
  const lower = name.toLowerCase();
  return !SNACK_KEYWORDS.some(kw => lower.includes(kw));
}

async function generateDosageAdvice(items) {
  // Сначала простой фильтр по ключевым словам — надёжно и без лишних токенов
  const courseItems = items.filter(i => isLikelyCourseItem(i.name));
  if (courseItems.length === 0) return [];

  // Подтягиваем реальную инструкцию с упаковки, если админ её указал в карточке товара —
  // тогда ИИ не гадает по общим знаниям, а форматирует то, что реально написано на упаковке
  const list = courseItems.map(i => {
    const product = i.product_id ? db.getProduct(i.product_id) : null;
    const instructions = product && product.dosage_instructions ? product.dosage_instructions.trim() : '';
    return instructions
      ? `- ${i.name} — РЕАЛЬНАЯ инструкция с упаковки (используй именно её, не придумывай другое): «${instructions}»`
      : `- ${i.name} — инструкции с упаковки нет, ориентируйся на общепринятые рекомендации для такого типа добавок`;
  }).join('\n');

  const prompt = `Ты — консультант магазина БАДов и спортпита. Пользователь купил следующие товары:
${list}

Твоя задача — дать рекомендацию по приёму, но ТОЛЬКО для товаров, которые принимаются курсом по расписанию (капсулы, таблетки, порошки/мерные ложки — витамины, БАДы, протеин, креатин и т.п.). Если товар — это батончик, снэк, готовый напиток, конфеты или что-то, что едят спонтанно по желанию, а не по расписанию — ПОЛНОСТЬЮ ИСКЛЮЧИ его из ответа, не включай в массив вообще.

Там, где указана РЕАЛЬНАЯ инструкция с упаковки — строго следуй именно ей при заполнении dosage_qty/timing/food_relation, не придумывай отличающиеся цифры.

Ответь СТРОГО в виде JSON-массива (может быть пустым [], если ни один товар не подходит), без пояснений до или после, без markdown-разметки. Формат каждого элемента:
...
Если для товара обычно достаточно приёма один раз в день — используй один элемент timing. Если по инструкции приём несколько раз в день — укажи несколько значений timing.`;
  ...
  try {
    ...
    return parsed
      .filter(p => p && p.name)
      .map(p => {
        // Противопоказания берём напрямую из карточки товара (не через ИИ) — точность тут важнее
        const matchedItem = courseItems.find(i => i.name === p.name || p.name.includes(i.name) || i.name.includes(p.name));
        const product = matchedItem?.product_id ? db.getProduct(matchedItem.product_id) : null;
        return {
          name: String(p.name),
