...
const TEMPLATE_META = {
  ...
  cart_nudge: { label: '«Корзина не оформлена»', hint: 'Через 30 минут после добавления в корзину, если заказ не оформлен', vars: ['items'] },
  ...
  restock_reminder: { label: '«Скоро закончится, докупить?»', hint: 'Когда до конца купленной пачки (по расчёту) остаётся несколько дней', vars: ['name', 'days_left'] },
  life_goals_question: { label: 'Вопрос про цели на жизнь', hint: 'Добавляется к самому первому ответу бота, когда человек впервые что-то ему написал', vars: [] },
